...
async def c55_student_webapp_submit(message: Message, bot: Bot):
    ...
    res = await execute_c55_webapp_payload(bot, message.from_user.id, kind, action, payload)
    if "data" in res:
        return await message.answer(
            f"<pre>{json.dumps(res['data'], ensure_ascii=False, indent=2)}</pre>",
            parse_mode="HTML",
        )
    return await message.answer(res["text"], parse_mode=res.get("parse_mode", "HTML"))
